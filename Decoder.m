...
subplot(211), hold on
if norm(y-round(y))<0.01, text(5,21, ['z(t)=[' int2str(y) ']']);
else text(5,21,['z(t)=[' num2str(y,'%5.1f') ']']);
end
subplot(212), hold on
...
for i=1:N
    ...
    if norm(y-round(y))<0.01, text(10*i+4,44, int2str(y(2*i-1:2*i)));
    else text(10*i+4,44,num2str(y(2*i-1:2*i),'%5.1f'));
    end
    
% State 1
    dis(1)=norm(y(2*i-1:2*i)-[0 0])^2+mu(4,i);
    dis(2)=norm(y(2*i-1:2*i)-[1 1])^2+mu(2,i);
...
% State 2
    dis(1)=norm(y(2*i-1:2*i)-[1 1])^2+mu(4,i);
    dis(2)=norm(y(2*i-1:2*i)-[0 0])^2+mu(2,i);
    j=find(dis==min(dis)); j=j(1);
...
% State 3
    dis(1)=norm(y(2*i-1:2*i)-[0 1])^2+mu(3,i);
    dis(2)=norm(y(2*i-1:2*i)-[1 0])^2+mu(1,i);
    j=find(dis==min(dis)); j=j(1);
...
% State 4
    dis(1)=norm(y(2*i-1:2*i)-[1 0])^2+mu(3,i);
    dis(2)=norm(y(2*i-1:2*i)-[0 1])^2+mu(1,i);
    j=find(dis==min(dis));  j=j(1);
